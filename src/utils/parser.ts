...
const IS_STRONG_CONSONANT = new RegExp(`[${strongConsonants.join('')}]`, 'i')
const IS_SPECIAL_CHARACTER = new RegExp(`[${specialCharacters.join('')}]`, 'i')
...
export function extractSyllables(word: string) {
  const splitByVowels = word.match(VOWEL_CONSONANT_SPLIT_REGEX)?.reverse() ?? []
    ...
    if (isNumber) {
      ...
      if (!phoneticVowels.includes(group)) {
        ...
        if (matches?.length === 2) {
          ...
          const vowel = matches
          currentSyllable = vowel.toString() + currentSyllable
        } else {
      ...
      } else if (IS_STRONG_CONSONANT.test(last)) {
